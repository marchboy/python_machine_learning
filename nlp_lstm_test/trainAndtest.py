...
d = pd.read_json(r'data.json') #训练数据已经被预处理成为标准json格式
...
#读入测试数据并进行分词
# dd = open(r'F:\品牌识别和核心实体识别\cas.txt', mode='r', encoding='UTF-8').read().decode('utf-8').split('\n')
dd = open(r'cas.1.txt', encoding='UTF-8').read().split(',')
...
output = TimeDistributed(Dense(5, activation='softmax',  activity_regularizer=regularizers.l1(0.01)))(blstm)
...
history = model.fit_generator(data_generator(d['words'], d['label'], batch_size), samples_per_epoch=1, nb_epoch=1)
