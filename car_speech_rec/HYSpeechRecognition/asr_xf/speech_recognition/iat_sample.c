...
//void run_iat(FILE* f_pcm, const char* session_begin_params)
{
...
int main(int argc, char* argv[])
{
       ...
       // int upload_on = 0; //是否上传用户词表
        const char* login_params = "appid = 5b695d8b, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	*/
	...
	// double start, end;
	//printf("%s\n", argv[1]);
	//run_iat("wav/REC0.WAV", session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
        run_iat(argv[1], session_begin_params);

    // for(int i=0; i<=40; i++)
	// {
	//     sprintf(fname, "wav/REC%d.WAV", i);
	//     // time(&first_time);
	//     // start_time = clock();
	//     gettimeofday(&tv, NULL);
	...
	//     run_iat(fname, session_begin_params);
